...
> **STATUS: SHIPPED (2026-04-23).** All tasks complete and merged via PRs #14, #16, #17, and #18. This plan is preserved as a historical reference — do NOT execute. See `git log` for per-task commits.
...
- [x] **Step 2: Baseline visual snapshots**
...
- [x] Add to `Gemfile` in the main group:
...
- [x] Add to `config/application.rb` inside `class Application < Rails::Application`:
...
- [x] Create the preview layout at `app/views/layouts/component_preview.html.erb` with Tailwind stylesheets loaded so previews render correctly.
...
- [x] Verify: `bundle exec rspec` still passes, `bin/rails routes | grep rails/view_components` shows the preview route mounted in dev.
...
- [x] Verify unused: `grep -r "container-custom" app/ config/` → expect 0 results.

- [x] Delete lines 17–21 (the `@layer components { .container-custom { @apply ... } }` block).

- [x] Run `bin/rails tailwindcss:build` and reload app. Spot-check any page looks identical.
...
- [x] Add `gem "rails_heroicon"` to `Gemfile`, `bundle install`.
- [x] Replace ONE inline SVG in `app/views/repositories/index.html.erb` (the empty-state archive box) with `<%= heroicon "archive-box", options: { class: "w-12 h-12 text-slate-300 dark:text-slate-600 mb-4" } %>` as a pilot.

**Option B — custom helper:**

- [x] Create `app/helpers/icon_helper.rb` with a `heroicon(name, class:)` method that reads SVG files from `app/assets/images/heroicons/outline/`.
- [x] Vendor needed icons only (archive-box, trash, check, chevron-right, chevron-down, lock-closed, magnifying-glass, clipboard, arrow-left, question-mark-circle, sun, moon).
...
- [x] **Red:** Write `spec/components/button_component_spec.rb` covering:
...
- [x] **Green:** Implement `ButtonComponent` per DESIGN.md Section 3.1. The `VARIANTS` and `SIZES` hashes copy DESIGN.md strings verbatim. Support three render modes via initializer: default `<button>`, submit (`type: :submit`), link (`href:`).

- [x] Run `bundle exec rspec spec/components/button_component_spec.rb` — all green.

- [x] Write `ButtonComponentPreview` with one preview method per variant × size combo.

- [x] Start `bin/dev`, visit `/rails/view_components`, verify all previews render correctly in both light and dark mode.
...
- [x] **Red:** Spec for variants (default / success / warning / danger / info / accent), `icon:` option (renders Heroicon before text with `gap-1.5`), content slot.

- [x] **Green:** Implement per DESIGN.md Section 3.5. Background floor `*-200` for light mode where DESIGN.md specifies `*-50` — use `*-200 text-*-800` to match the Light Mode Minimum Contrast Rule (this resolves the internal DESIGN.md conflict noted in the review).

- [x] Preview all six variants × (with icon / without icon) combos.
...
- [x] **Red:** Spec for `size:` (sm `h-8` / md `h-10` / lg `h-12`), `type:` (text/email/password/search), `error:` state (red border, aria-describedby), `help_text:`, `label:`, `required:` (red asterisk).

- [x] **Green:** Implement per DESIGN.md Section 3.3. Label uses `text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5`.
...
- [x] Spec for `options:` (array of [label, value]), `selected:`, `size:` (same h-* tokens as Input), `label:`, `help_text:`, `error:`.
- [x] Match Input's height tokens exactly so inline groupings align (DESIGN.md 3.3 Inline Height Consistency Rule).
...
- [x] Spec for `rows:`, `label:`, `placeholder:`, `error:`, `help_text:`.
- [x] Textarea uses `py-2.5 resize-y` — no `h-*` since it's multi-line (explicit exception from Input rule).
...
- [x] **Red:** Spec for optional `header:` slot, required default content (body), optional `footer:` slot, optional `padding: :none` to let callers control inner padding (e.g., table rows).
- [x] **Green:** Implement per DESIGN.md Section 3.2. Default padding `p-6`, header `px-6 py-4 border-b`, footer `px-6 py-4 border-t bg-slate-50/50`.
- [x] Preview: basic card, card with header, card with header + footer, card with `padding: :none` + embedded DataList.
...
- [x] Replace the outer `link_to` wrapper's Tailwind classes with a `CardComponent` wrapped in `link_to`, passing `class:` to the link.
- [x] Replace the "Docker Image" inline badge with `<%= render BadgeComponent.new(variant: :info) { "Docker Image" } %>`.
- [x] Keep the repository-name heading, metadata row, and chevron icon as-is (they're display content, not components).
- [x] Verify: compare screenshots from Pre-flight Step 2 — pixel-diff should show no change.
- [x] Run `bundle exec rspec spec/features/repositories_index_spec.rb` (or equivalent system test).
...
- [x] Replace search bar's raw `f.text_field` and `f.select` with `InputComponent` and `SelectComponent` (size `:md` for both — already matches current `h-10`).
- [x] Wrap the search bar in `CardComponent.new(padding: :md)` (outer `div.rounded-lg.bg-white...`).
- [x] Extract empty-state markup into a new partial or keep inline — either works, but note the structure will be reused in `index.turbo_stream.erb` (Task 18).
- [x] Verify visual diff is empty.
...
- [x] Top info card → `CardComponent`.
- [x] "Docker pull command" row → keep as inline `<div>` (specialized clipboard component, not a Card).
- [x] Back nav link → keep as `link_to` with icon (it's navigation, not an action button).
- [x] `<details>` Edit section:
...
- [x] Desktop tags grid: keep CSS Grid structure, but Protected badge → `BadgeComponent.new(variant: :warning)` with `"Protected"` content. **NOTE:** Do not fix emoji / text-xs yet — preserve the violation; Task 19 fixes it. In this commit, pass `"🔒 Protected"` literally to preserve identical output.
- [x] Tags grid: Delete button → `ButtonComponent.new(variant: :link, size: :sm, icon: "trash", href: ...)` or `button_to` wrapper with ButtonComponent-equivalent classes.
- [x] Mobile card stack: same component swaps. Preserve the missing-icon bug on Delete (Task 20 will fix).
- [x] Bottom "Delete Repository" button → `ButtonComponent.new(variant: :danger, icon: "trash")`.
...
- [x] Info card → `CardComponent`.
- [x] Layers card → `CardComponent` with `padding: :none` + header slot for the "Layers (N)" title.
- [x] Docker Config card → `CardComponent`.
- [x] Top-level Delete Tag button → `ButtonComponent.new(variant: :danger, icon: "trash")`.
- [x] Preserve `🔒 Protected` / `text-xs` for now (Task 19 fix).
...
- [x] Each event wrapper → `CardComponent`.
- [x] Action badge (create/update/delete) → `BadgeComponent.new(variant: mapping)` where mapping is `create: :success, update: :warning, delete: :danger`.
- [x] Empty state — keep inline (same pattern as index).
...
- [x] Each section wrapper → `CardComponent`.
- [x] Final "Supported Image Formats" alert block: keep as custom warning panel (amber-100 bg + border) — this is a specialized callout, not a Card variant. Alternatively, add a `CardComponent` variant `:warning` in a follow-up. **For this task: leave inline.**

**Commit:** `refactor(views): migrate help/show to components`

### Task 16: Phase B checkpoint — full regression

- [x] Run the full test suite: `bundle exec rspec && bundle exec rspec spec/system`.
- [x] Re-capture screenshots for all pages at all viewports and compare to baselines (Pre-flight Step 2). Any visual diff is a bug — fix before proceeding to Phase C.
- [x] Verify `/rails/view_components` previews all render.
...
- [x] **Red:** Add a system spec that visits a tag page where the tag is protected and asserts:
...
- [x] Verify: run the spec, inspect the rendered page at `/repositories/:name` with a protected tag.
...
- [x] **Red:** Add a request spec at `spec/requests/repositories_search_spec.rb`:
...
- [x] **Red:** System spec asserting the Save button has an SVG child (icon).
- [x] **Green:** Update the `ButtonComponent` call from Task 12 to pass `icon: "check"`.
- [x] If Task 12 already added the icon (non-strict path), skip this task.
...
- [x] **Red:** System spec at mobile viewport (375px) — `resize_window_to(375, 800)` — asserting the Delete button on the tag list has a trash SVG.
- [x] **Green:** In the mobile card Delete button (`button_to` in the `md:hidden` block), change the `ButtonComponent.new(variant: :link, size: :sm)` call to include `icon: "trash"`. Match the desktop version.
...
- [x] **Red:** Visual regression — manually verify before/after: labels "Description" and "Maintainer" should use `text-slate-700 dark:text-slate-300` (label role) per DESIGN.md Visual Hierarchy table.
- [x] **Green:** Change `text-slate-500 dark:text-slate-400` → `text-slate-700 dark:text-slate-300` on the two label `<p>` elements.
...
- [x] Verify via keyboard: Tab to the summary, confirm visible focus ring in both light and dark mode.
...
- [x] Discuss with team: wide-monitor readability (current) vs data density (DESIGN.md-preferred full width).
- [x] If removing `container mx-auto`: update both `<nav>` wrapper and `<main>` wrapper consistently so edges still align.
- [x] Document the chosen direction in `docs/standards/STACK.md`.
...
- [x] **Step V1: Full test suite green**
...
- [x] **Step V4: Turbo Stream response**
