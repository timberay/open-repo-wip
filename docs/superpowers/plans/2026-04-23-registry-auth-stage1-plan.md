...
**PR-1:** merged (branch `feature/registry-auth-stage1-pr1` — structural `actor:` kwarg 도입 + initializer).
**PR-2:** merged — [PR #30](https://github.com/timberay/open-repo/pull/30), 17 commits, full suite 406 runs / 0 failures.

**Scope deviations actually shipped vs. original plan:**

1. **Task 2.8 — Critical Gap #3 scenarios: 10 of 11 (not 11).** Scenario 6 (`GET /v2/:name/blobs/:digest → 200`) skipped with in-file rationale: `V2::BlobsController#show` requires a real blob on disk (`BlobStore#exists?` is a filesystem check); Minitest 6.0.4 has no `stub_any_instance` and Mocha is not used; auth-gate path for `blobs/show` is identical to `manifests/show` (same `ANONYMOUS_PULL_ENDPOINTS` whitelist entry) so coverage redundancy is low-value.

2. **Task 2.10 — ImportsController wiring (Steps 5–7): deferred.** `app/controllers/imports_controller.rb` does not exist in this codebase and `ProcessTarImportJob.perform_later` has zero production callers. The job's new `actor_email:` kwarg defaults to `nil` → `"system:import"` fallback, matching prior behavior. A future ImportsController can pass a real email.

3. **Task 2.12 — NavComponent → ERB partial edit.** Project has no `NavComponent` ViewComponent; nav lives at `app/views/shared/_auth_nav.html.erb`. Tokens link added there directly.

4. **Task 2.12 — System test → integration test.** Project has no `test/system/` directory, no `ApplicationSystemTestCase`, no Selenium. System test replaced with 2 integration tests appended to `test/integration/login_button_visibility_test.rb`. End-to-end PAT CRUD flow (sign in → create → revoke → revoked PAT can't push) is already covered by `test/controllers/settings/tokens_controller_test.rb`'s `reset!` + Basic-auth-on-V2 scenario.

5. **Task 2.11 — plan narrative said "Expected: 8 PASS" but the plan body lists 9 tests.** 9 delivered, 9 pass.

**Deferred to Phase 3 (post-merge):**
- Staging canary checklist (`### Task 3.1`) — requires devops environment access.
- Production deploy + 15-minute canary monitoring.
- CI / K8s ImagePullSecret PAT rotation.

**Known follow-up:** `config/credentials.yml.enc` carried unstaged through the session (Stage 0 artifact, unrelated to Stage 1). Address separately if encryption key rotation is needed.
...
**주의:**
- `ActionController::HttpAuthentication::Basic.user_name_and_password` 는 Rails 8.1 에서 예외 대신 nil 반환 (malformed base64 는 lenient `Base64.decode64` → garbage bytes → split 후 nil). `email.blank? || raw.blank?` 이 모든 malformed 케이스 커버 (이전 plan 의 `ActionController::HttpAuthentication::Basic::HttpBasicAuthenticationError` 상수는 Rails 8.1 에 존재하지 않음 — 제거됨).
- `update_column` (not `update!`) — validations skip + updated_at 변경 방지. 순수 last_used_at write.
- Response header 이름은 **`Docker-Distribution-API-Version`** (API 대문자) — Docker Distribution reference 구현체와 일치. `set_registry_headers` 의 기존 header 와 동일하게 casing 통일.
...
| 항목 | 확인 방법 | 상태 (2026-04-24) |
|---|---|---|
| 2 PR 모두 main merge | `gh pr list --base main --state merged --search "Stage 1"` | ✅ PR-1 + PR #30 |
| `bin/rails test` green | `bin/rails test` | ✅ 406 runs, 0 failures |
| `bin/rails test:system` green | `bin/rails test:system` | ⊘ N/A — `test/system/` 미존재 (integration test 로 대체, Completion Status 의 Task 2.12 편차 참조) |
| Critical gap hard gate green | CI 로그 | ✅ `anonymous_pull_regression` + `docker_basic_auth` |
| Staging 1일 soak pass | V2 push 지표 > 95% | ⏳ Phase 3 (devops 환경 필요) |
| Production canary 15분 pass | kamal 배포 후 모니터링 로그 | ⏳ Phase 3 |
| CI / K8s PAT 교체 완료 | devops 확인 | ⏳ Phase 3 |
| `TagEvent.actor` 실명 기록 중 | 실시간 DB 확인 | ⏳ Phase 3 (staging DB) |
| Slack 공지 발송 | "Stage 1 배포 순간 모든 push/delete 가 PAT 필수" | ⏳ Phase 3 |
